...
// This file doesn't really make any sense.  It has actual code in a .h file, with some C++ objects
// defined.  It was originally based on an nvidia file that should have been left unmodified, but
// has a lot of code added.  I'd really like to clean this up, but since 3D Vision is dead, it
// won't really matter, so leaving as is, with minor editing for style.
...
static void get_primary_display_resolution(
    float& pWidth,
    float& pHeight)
{
    NvAPI_Status ret;
    ...
    pathInfo = static_cast<NV_DISPLAYCONFIG_PATH_INFO*>(malloc(pathCount * sizeof(NV_DISPLAYCONFIG_PATH_INFO)));
    if (!pathInfo)
    {
    ...
    for (NvU32 i = 0; i < pathCount; i++)
    {
        ...
        pathInfo[i].sourceModeInfo = static_cast<NV_DISPLAYCONFIG_SOURCE_MODE_INFO*>(malloc(sizeof(NV_DISPLAYCONFIG_SOURCE_MODE_INFO)));
    ...
    for (NvU32 i = 0; i < pathCount; i++)
    {
        ...
        {
            pWidth  = static_cast<float>(pathInfo[i].sourceModeInfo->resolution.width);
            pHeight = static_cast<float>(pathInfo[i].sourceModeInfo->resolution.height);
        }
...
inline void PopulateTextureData(
    float*                leftEye,
    float*                rightEye,
    ...
    float                 sep,
    float                 conv,
    float                 tuneValue1   = 0.0f,
    float                 tuneValue2   = 0.0f,
    float                 tuneValue3   = 0.0f,
    float                 tuneValue4   = 0.0f,
...
struct D3D9Type
{
    ...
    static StagingResource* CreateStagingResource(Device* pDevice, float eyeSep, float sep, float conv, float tuneValue1, float tuneValue2, float tuneValue3, float tuneValue4)
    {
...
struct d3d11_type
{
    ...
    static constexpr NV_STEREO_REGISTRY_PROFILE_TYPE RegistryProfileType = NVAPI_STEREO_DX10_REGISTRY_PROFILE;

    // Note that the texture must be at least 20 bytes wide to handle the stereo header.
    static constexpr int         StereoTexWidth      = 8;
    static constexpr int         StereoTexHeight     = 1;
    static constexpr DXGI_FORMAT StereoTexFormat     = DXGI_FORMAT_R32G32B32A32_FLOAT;
    static constexpr int         StereoBytesPerPixel = 16;

    static StagingResource* CreateStagingResource(
        Device* pDevice,
        float   eyeSep,
        float   sep,
        float   conv,
        ...
        float   tuneValue4)
    {
        ...
        get_primary_display_resolution(screenWidth, screenHeight);
        ...
        float*                leftEyePtr  = static_cast<float*>(const_cast<void*>(sysData.pSysMem));
        float*                rightEyePtr = leftEyePtr + StereoTexWidth * StereoBytesPerPixel / sizeof(float);
        LPNVSTEREOIMAGEHEADER header      = reinterpret_cast<LPNVSTEREOIMAGEHEADER>(static_cast<unsigned char*>(const_cast<void*>(sysData.pSysMem)) + sysData.SysMemPitch * StereoTexHeight);
        PopulateTextureData(leftEyePtr, rightEyePtr, header, stagingWidth, stagingHeight, StereoBytesPerPixel, eyeSep, sep, conv, tuneValue1, tuneValue2, tuneValue3, tuneValue4, screenWidth, screenHeight);
    ...
    static void UpdateTextureFromStaging(
        Context*         context,
        Texture*         tex,
...
public:
        ...
        bool deviceLost)
    {
        ...
        if (active)
        {
            ...
            updateRequired = (eyeSep != mEyeSeparation) || (sep != mSeparation) || (conv != mConvergence) || (active != mActive);
        }
        else
        {
            eyeSep = sep = conv = 0;
    ...
    float GetConvergenceDepth() const
    {
        return mActive ? mConvergence : 1.0f;
    }

    // 3DMIGOTO ADDITION: In DX9 or DX10 pass the device a second time as the context
    bool UpdateStereoTexture(
        Device*  dev,
        Context* context,
        Texture* tex,
        bool     deviceLost)
    {
        ...
        StagingResource* staging = D3DType::CreateStagingResource(dev, mEyeSeparation, mSeparation * mSeparationModifier, mConvergence, mTuneVariable1, mTuneVariable2, mTuneVariable3, mTuneVariable4);
        if (staging)
        {
...
typedef ParamTextureManager<D3D10Type> ParamTextureManagerD3D10;
    ...
    #ifndef NO_STEREO_D3D11  // 3DMIGOTO ADDITION
typedef ParamTextureManager<d3d11_type> ParamTextureManagerD3D11;
    #endif
};  // namespace stereo
};  // namespace nv
