...
static void SimpleScreenShot(HackerDevice *pDevice, UINT64 hash, wstring shaderType)
{
	...
	hr = G->gHackerSwapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), (LPVOID*)&backBuffer);
	if (SUCCEEDED(hr))
	{
...
static void StereoScreenShot(HackerDevice *pDevice, UINT64 hash, wstring shaderType)
{
	...
	hr = G->gHackerSwapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), (void**)&backBuffer);
	if (FAILED(hr))
		return;
