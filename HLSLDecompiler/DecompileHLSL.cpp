...
#pragma warning(disable: 6328)
	...
	void applySwizzle(const char *left, char *right, bool useInt = false)
	{
		...
		else if (right[0] == 'c' && right[1] != 'b')	//dx9µÄ³£Á¿¼Ä´æÆ÷£¬c¿ªÍ·
		{

			char * result = strrchr(right, '.');
			if (result == NULL)		//Èç¹ûÃ»ÓÐswizzleÐÅÏ¢£¬¼ÓÉÏ.xyzw
			{
				strcat(right, ".xyzw");
			}
					...
					case OPCODE_DP4:
						if (shader->dx9Shader)
						{
							...
							if (nextInstr->eOpcode == OPCODE_RSQ && outputOp0.size() == 3)
							{
								...
								//asmÀïÖ»ÓÐÒ»ÐÐ£¬ËùÒÔ²»ÐèÒªµ÷ÓÃReadStatement
								//Ö¸ÁîÓÐÁ½Ìõ
								iNr++;
							}
							else
