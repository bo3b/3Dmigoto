...
{
    LOG_DEBUG("D3D9Wrapper::IDirect3D9::QueryInterface called\n");// at 'this': %s\n", type_name_dx9((IUnknown*)this));
    HRESULT hr = NULL;
...
{
    LOG_DEBUG("IDirect3D9::Release handle=%p, counter=%lu, this=%p\n", m_pUnk, m_ulRef, this);
    ...
    if (ulRef == 0)
    {
        if (!gLogDebug) LOG_INFO("IDirect3D9::Release handle=%p, counter=%lu, internal counter = %lu\n", m_pUnk, m_ulRef, ulRef);
...
{
    LOG_INFO("IDirect3D9::RegisterSoftwareDevice called\n");
...
{
    LOG_INFO("IDirect3D9::GetAdapterCount called\n");

    UINT ret = GetDirect3D9()->GetAdapterCount();
    LOG_INFO("  return value = %d\n", ret);
...
{
    LOG_INFO("IDirect3D9::GetAdapterModeCount called\n");
...
{
    LOG_INFO("IDirect3D9::EnumAdapterModes called: adapter #%d requested with mode #%d\n", Adapter, Mode);

    HRESULT hr = GetDirect3D9()->EnumAdapterModes(Adapter, Format, Mode, pMode);
    if (hr == S_OK)
    {
        LOG_INFO("  driver returned width=%d, height=%d, refresh rate=%d\n", pMode->Width, pMode->Height, pMode->RefreshRate);

        /*
        if (SCREEN_REFRESH >= 0 && pMode->RefreshRate != SCREEN_REFRESH)
        {
            LOG_INFO("  video mode ignored because of mismatched refresh rate.\n");

            pMode->Width = pMode->Height = 0;
        }
        else if (SCREEN_WIDTH >= 0 && pMode->Width != SCREEN_WIDTH)
        {
            LOG_INFO("  video mode ignored because of mismatched screen width.\n");

            pMode->Width = pMode->Height = 0;
        }
        else if (SCREEN_HEIGHT >= 0 && pMode->Height != SCREEN_HEIGHT)
        {
            LOG_INFO("  video mode ignored because of mismatched screen height.\n");
...
{
    LOG_INFO("IDirect3D9::GetAdapterDisplayMode called: adapter #%d\n", Adapter);
...
//        {
//            LOG_INFO("  overriding Width %d with %d\n", pMode->Width, G->SCREEN_WIDTH);
//            pMode->Width = G->SCREEN_WIDTH;
//        }
...
{
    LOG_INFO("IDirect3D9::CheckDeviceType called with adapter=%d\n", Adapter);
...
{
    LOG_INFO("IDirect3D9::CheckDeviceFormat called with adapter=%d\n", Adapter);
...
{
    LOG_INFO("IDirect3D9::CheckDeviceMultiSampleType called: adapter #%d\n", Adapter);
...
{
    LOG_INFO("IDirect3D9::CheckDepthStencilMatch called: adapter #%d\n", Adapter);
...
{
    LOG_INFO("IDirect3D9::CheckDeviceFormatConversion called: adapter #%d\n", Adapter);
...
{
    LOG_INFO("IDirect3D9::GetDeviceCaps called: adapter #%d\n", Adapter);
...
{
    LOG_INFO("IDirect3D9::GetAdapterMonitor called: adapter #%d\n", Adapter);
...
{
    LOG_INFO("IDirect3D9::CreateDeviceEx called with parameters FocusWindow = %p\n", hFocusWindow);
    ...
    if (pPresentationParameters != NULL) {
        ...
        LOG_INFO("Got resolution from device creation: %ix%i\n",
    ...
    {
        LOG_INFO("  postponing device creation and returning only wrapper\n");
    }
    ...
    if (newDevice == NULL)
    {
        LOG_INFO("  error creating IDirect3DDevice9 wrapper.\n");
...
{
    LOG_INFO("IDirect3D9::CreateDevice called: adapter #%d\n", Adapter);

    if (G->gForwardToEx) {
        LOG_INFO("  forwarding to CreateDeviceEx.\n");
...
{
    LOG_INFO("IDirect3D9::GetAdapterModeCountEx called: adapter #%d\n", Adapter);
...
{
    LOG_INFO("IDirect3D9::EnumAdapterModesEx called: adapter #%d\n", Adapter);
...
{
    LOG_INFO("IDirect3D9::GetAdapterDisplayModeEx called: adapter #%d\n", Adapter);
...
{
    LOG_INFO("IDirect3D9::GetAdapterLUID called: adapter #%d\n", Adapter);
