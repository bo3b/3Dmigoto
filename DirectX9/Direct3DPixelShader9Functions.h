...
{
	LogDebug("IDirect3DPixelShader9::Release handle=%x, counter=%d, this=%x\n", m_pUnk, m_ulRef, this);
    ...
    if (ulRef == 0)
    {
		if (!gLogDebug) LogInfo("IDirect3DPixelShader9::Release handle=%x, counter=%d, internal counter = %d\n", m_pUnk, m_ulRef, ulRef);
