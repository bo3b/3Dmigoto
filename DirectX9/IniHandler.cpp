...
void LoadConfigFile()
{
	...
	G->assemble_signature_comments = GetIniBool(L"Rendering", L"assemble_signature_comments", false, NULL, &migoto_ini);
	...
	if (GetIniStringAndLog(L"Rendering", L"fix_ZRepair_Dependencies2", 0, setting, MAX_PATH, &migoto_ini))
	{
		char buf[MAX_PATH];
		wcstombs(buf, setting, MAX_PATH);
		char *start = buf; while (isspace(*start)) ++start;
	...
	if (GetIniStringAndLog(L"Rendering", L"fix_InvTransform", 0, setting, MAX_PATH, &migoto_ini))
	{
		char buf[MAX_PATH];
		wcstombs(buf, setting, MAX_PATH);
		char *start = buf; while (isspace(*start)) ++start;
	...
	if (GetIniStringAndLog(L"Rendering", L"fix_ZRepair_DepthTextureHash", 0, setting, MAX_PATH, &migoto_ini))
	{
		uint32_t hash;
