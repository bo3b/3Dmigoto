...
void InputListener::UpEvent(D3D9Wrapper::IDirect3DDevice9 *device)
{
}
...
void InputCallbacks::DownEvent(D3D9Wrapper::IDirect3DDevice9 *device)
{
	if (down_cb)
		return down_cb(device, private_data);
}

void InputCallbacks::UpEvent(D3D9Wrapper::IDirect3DDevice9 *device)
{
	if (up_cb)
...
bool InputAction::Dispatch(D3D9Wrapper::IDirect3DDevice9 *device)
{
...
bool RepeatingInputAction::Dispatch(D3D9Wrapper::IDirect3DDevice9 *device)
{
...
bool DelayedInputAction::Dispatch(D3D9Wrapper::IDirect3DDevice9 *device)
{
	...
	void *private_data)
{
	...
	if (!GetIniString(app, iniKey, 0, keyName, MAX_PATH, &migoto_ini))
		return false;
...
bool DispatchInputEvents(D3D9Wrapper::IDirect3DDevice9 *device)
{
	std::vector<class InputAction *>::iterator i;
	class InputAction *action;
